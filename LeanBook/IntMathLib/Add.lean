...
@[simp] theorem MyInt.add_def (x₁ x₂ y₁ y₂: MyNat)
...
@[simp] theorem MyInt.zero_add (m: MyInt) : 0 + m = m := by
  refine Quotient.inductionOn m ?_
  intro (a₁, a₂)
  apply Quotient.sound
  notation_simp
...
example (a b: MyInt): (a + b) - b = a := by abel

example (a b c: MyInt) : (a - b) - c + b + c = a := by abel
