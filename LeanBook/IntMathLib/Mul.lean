...
example (a b c: MyNat): (a + b) * (a + c) = a * a + (b + c) * a + b * c := by ring
...
@[notation_simp] theorem MyNat.toMyNat_one : MyNat.ofNat 1 = 1 := rfl

@[simp] theorem MyInt.mul_one (m: MyInt) : m * 1 = m := by
  revert m
  unfold_int₁
  ring

@[simp] theorem MyInt.one_mul (m: MyInt) : 1 * m = m := by
  revert m
  unfold_int₁
  ring

@[simp] theorem MyInt.mul_zero (m: MyInt) : m * 0 = 0 := by
  revert m
  unfold_int₁

@[simp] theorem MyInt.zero_mul (m: MyInt) : 0 * m = 0 := by
  revert m
  unfold_int₁
  ring
...
example (m n: MyInt): (m - n) * (m + n) = m * m - n * n := by ring
